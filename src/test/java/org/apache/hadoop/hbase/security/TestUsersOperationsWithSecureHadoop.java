...
public class TestUsersOperationsWithSecureHadoop {
   ...
   */
  @Test
  public void testLoginInNonSecureHadoop() throws Exception {
    User.IS_SECURE_HADOOP = false;
    try {
      UserGroupInformation defaultLogin = UserGroupInformation.getLoginUser();
      User.login(HBaseConfiguration.create(), "fileConfKey",
          "principalConfKey", "localhost");
      assertTrue("Login has no effect in non secure hadoop",
          defaultLogin.equals(UserGroupInformation.getLoginUser()));
    } catch (IOException ioException) {
      throw new Exception("error in testLoginInNonSecureHadoop", ioException);
    } finally {
